...
function renderProductPanel(product, platform) {
  ...
  // Hide product grid and headers to simulate a new page
  document.querySelectorAll('.products-grid, .page-title, .toolbar').forEach(el => el.style.display = 'none');
  ...
  panel.innerHTML = `
    ...
    <div style="margin-bottom: 20px;">
      <button onclick="backToGrid()" style="padding: 8px 16px; background: transparent; border: 1px solid #d1d5db; border-radius: 6px; cursor: pointer; font-weight: 600; color: #374151; display: inline-flex; align-items: center; gap: 8px;">
        <span style="font-size: 16px;">&larr;</span> Back to Products
      </button>
    </div>
    ` : ''}
...
window.backToGrid = function() {
  const panel = document.getElementById('prism-product-panel');
  if (panel) panel.style.display = 'none';
  
  // Restore grid and headers
  document.querySelectorAll('.products-grid').forEach(el => el.style.display = 'grid');
  document.querySelectorAll('.page-title').forEach(el => el.style.display = 'block');
  document.querySelectorAll('.toolbar').forEach(el => el.style.display = 'flex');
  
  // Remove active state from cards
  document.querySelectorAll('.product-grid-card').forEach(c => c.classList.remove('active'));
  window.scrollTo({ top: 0, behavior: 'smooth' });
};
...
async function initProductCommerceExperience(platform) {
  ...
  bootstrapReviews(platform);
  updateLiveCounter();
  // Products grid is shown by default now; no auto-open.
}
