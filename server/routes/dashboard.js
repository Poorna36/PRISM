...
function parseReviewFeaturesForPrimary(featuresField) {
  ...
  let f;
  try {
    f = typeof featuresField === 'string' ? JSON.parse(featuresField) : featuresField;
  } catch {
    return null;
  }
...
/**
 * Heuristics for demo charts: batch spikes (one-week surges) vs design drift (multi-week climb).
 */
...
router.get('/all', async (req, res) => {
  try {
    ...
    const leaderboard = Object.entries(byProduct).map(([pid, prs]) => {
...
router.get('/:productId', async (req, res) => {
  try {
    ...
    const { platform, week_range = 24 } = req.query;
    ...
    const sortedWeeks = Object.keys(byWeek).sort().slice(-parseInt(week_range));
    ...
    // Graph data — primary feature from Review.features (similarity order), not arbitrary array order
    ...
    for (const r of reviewRows) {
      reviewFeatureMap[r.id] = r.features;
    }

    const edges =
      nodeIds.length > 0
        ? await GraphEdge.findAll({
            where: { product_id: productId, source_node_id: { [Op.in]: nodeIds } },
          })
        : [];
    ...
    return res.json({
      ...
      graph_data: {
        nodes: nodes.map((n) => {
          ...
          let sentiments = {};
          try {
            sentiments = n.sentiments ? JSON.parse(n.sentiments) : {};
          } catch {
            sentiments = {};
          }
          const sentiment =
            primary && sentiments[primary] != null
              ? sentiments[primary]
